...
gdp <- read.csv("Data/WB_gdpconst.csv", skip = 3) %>%
...
pop <- read.csv("Data/WB_totpop.csv", skip=3) %>%
  filter(Country.Name %in% EU31) %>%
  select(c(1,13:63)) %>%
...
transp.em <- read_excel("Data/v50_CO2_excl_short-cycle_org_C_1970_2018.xls",
...
#### Output
write.csv(data, "CO2DriversEU_dataset.csv", row.names = F)
